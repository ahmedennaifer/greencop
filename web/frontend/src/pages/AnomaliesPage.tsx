        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-gradient-to-br from-red-50 to-red-100 border-red-200">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1">
              <CardTitle className="text-xs font-medium text-red-900">Total Anomalies</CardTitle>
              <AlertTriangle className="h-4 w-4 text-red-600" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-red-600">{stats?.total_anomalies || 0}</div>
              <p className="text-xs text-red-600 mt-0.5">All time</p>
            </CardContent>
          </Card>

          <Card className="bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1">
              <CardTitle className="text-xs font-medium text-orange-900">Last 24 Hours</CardTitle>
              <Activity className="h-4 w-4 text-orange-600" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">{stats?.last_24h || 0}</div>
              <p className="text-xs text-orange-600 mt-0.5">Recent activity</p>
            </CardContent>
          </Card>

          <Card className="bg-gradient-to-br from-yellow-50 to-yellow-100 border-yellow-200">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1">
              <CardTitle className="text-xs font-medium text-yellow-900">Last 7 Days</CardTitle>
              <Activity className="h-4 w-4 text-yellow-600" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-yellow-600">{stats?.last_7d || 0}</div>
              <p className="text-xs text-yellow-600 mt-0.5">Weekly trend</p>
            </CardContent>
          </Card>
        </div>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={dailyData}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="count" stroke="#4ade80" strokeWidth={2} name="Anomalies" dot={{ fill: '#4ade80', r: 5 }} />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={sensorData} layout="vertical">
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="count" fill="#8b5cf6" name="Anomalies" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
