...
def generate_training_data(days=7, anomaly_rate=0.05):
    ...
    for i in range(num_samples):
        ...
        if is_anomaly:
            anomaly_type = random.choice(["critical_temp", "critical_humidity", "warning_temp", "warning_humidity", "both"])
            ...
            if anomaly_type == "critical_temp":
                ...
                humidity = round(random.choice([random.uniform(0, 20), random.uniform(80, 95)]), 2)
                label = -1
                ...
                temperature = round(base_temp + temp_cycle + temp_noise, 2)
                humidity = round(random.choice([random.uniform(20, 40), random.uniform(60, 80)]), 2)
                label = -1
            else:
                temperature = round(random.uniform(32.0, 50.0), 2)
                humidity = round(random.choice([random.uniform(0, 20), random.uniform(80, 95)]), 2)
    ...
    df['temp_rolling_mean'] = df['temperature'].rolling(window=360, min_periods=1).mean()
    df['humidity_rolling_mean'] = df['humidity'].rolling(window=360, min_periods=1).mean()
...
if __name__ == "__main__":
    ...
    upload_to_gcs(df, "atomic-climate-482314-q7-ml-models", "training_data/sensor_data_7days.csv")
