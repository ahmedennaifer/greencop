...
async def predict_anomaly(sensor_id: str, lookback: int = Query(100, le=500)):
    import numpy as np
    import requests

    ML_PREDICT_URL = os.environ.get(
        "ML_PREDICT_URL", "https://ml-predict-x6j3tvo2ca-ew.a.run.app"
    )
    ...
    try:
        ...
        steps_ahead = 5
        predicted_temp_short = temps[-1] + (avg_temp_delta * steps_ahead)
        predicted_hum_short = hums[-1] + (avg_hum_delta * steps_ahead)

        from datetime import datetime

        current_time = datetime.fromisoformat(
            str(results[-1].timestamp).replace("Z", "+00:00")
        )
        ...
        for seconds in [5, 10, 15, 20]:
            readings_ahead = max(1, seconds // 2)

            pred_temp = temps[-1] + (avg_temp_delta * readings_ahead)
            pred_hum = hums[-1] + (avg_hum_delta * readings_ahead)

            pred_temp = max(min(pred_temp, 100), -50)  # -50°C to 100°C
            pred_hum = max(min(pred_hum, 100), 0)  # 0% to 100%

            short_predictions.append(
                {
        ...
        return {
            ...
            "predicted_temp": round(predicted_temp_short, 1),
            "predicted_humidity": round(predicted_hum_short, 1),
            "short_predictions": short_predictions,
            "trend": {
                "temp_delta_per_reading": round(avg_temp_delta, 4),
                "humidity_delta_per_reading": round(avg_hum_delta, 4),
                "temp_delta_per_second": round(
                    avg_temp_delta / 2, 4
                ),  # ~2 sec per reading
                "humidity_delta_per_second": round(avg_hum_delta / 2, 4),
            },
            "message": "Anomaly predicted in next 5-10 seconds"
            if prediction == -1
            else "Normal behavior expected",
        }
